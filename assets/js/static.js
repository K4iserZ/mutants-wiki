...
async function initMutantsPage() {
  const page = document.getElementById('pageContent'); page.innerHTML = `<section class="section section--tight"><div class="section__head"><h2>Mutants</h2><p class="section__lede">All avaialble mutants.</p></div><div class="filters" id="filters"><button class="filter is-active" data-filter="all">All</button><button class="filter" data-filter="legendary">Legendarios</button><button class="filter" data-filter="heroic">Heroic</button><button class="filter" data-filter="recipe">Secrets</button><button class="filter" data-filter="common">Comunes</button></div><div id="mutantGrid" class="cardgrid cardgrid--full"><p class="loading">Loading mutants...</p></div></section>`;
...
async function initMutantDetailPage(slug) { const page = document.getElementById('pageContent'); page.innerHTML = '<section class="detail" id="mutantDetail"><p class="loading">Cargando ficha del mutante...</p></section>'; try { const catalog = await loadIndex(); const specimen = catalog.find(item => slugify(item.name) === slug.toLowerCase()); if (!specimen?.specimen) throw new Error('Specimen no encontrado'); document.title = `MGG Wiki | ${specimen.name}`; let localDetail = {}; try { const localResponse = await fetch(`${DATA_BASE}/${encodeURIComponent(slug)}.json`); if (localResponse.ok && localResponse.headers.get('content-type')?.includes('application/json')) localDetail = await localResponse.json(); } catch { localDetail = {}; } const apiUrl = API_CONFIG.api.specimen.replace('{specimen}', encodeURIComponent(specimen.specimen)); const apiResponse = await fetch(apiUrl); if (!apiResponse.ok) throw new Error('No se pudo cargar el specimen desde la API'); const apiDetail = await apiResponse.json(); const abilitiesConfig = await loadAbilitiesConfig(); const gachaConfig = await loadGachaConfig(); await loadOrbsConfig(); renderDetail(document.getElementById('mutantDetail'), normalizeSpecimenDetail(apiDetail, localDetail, specimen, abilitiesConfig, gachaConfig)); } catch (error) { document.getElementById('mutantDetail').innerHTML = errorMessage(error); } }
...
function breedableLabel(value) { return ({ yes: 'Sí', no: 'No', if_owned: 'Si ya lo tienes' }[value] || value || '-'); }
function escapeHtml(value) { const div = document.createElement('div'); div.textContent = value ?? ''; return div.innerHTML; }
function errorMessage(error) { return `<p class="loading">${escapeHtml(error.message || 'No se pudo cargar la información.')}</p>`; }
...
function addLarvaSection(container, mutant) {
  ...
  larva.innerHTML = `<h2>Larva</h2><div class="larva-art"><img src="${larvaImage}" alt="Larva de ${escapeHtml(mutant.name)}" onerror="this.hidden=true"></div>`;
...
function orbSlotMarkup(slotString, selections, mutant) {
  ...
  return slots.map((slot, index) => {
    ...
    return `<div class="statbox statbox--additional orb-slot orb-slot--visual" data-orb-slot="${index}" data-orb-kind="${kind}"><span class="orb-slot__label">Orbe ${index + 1}</span><button class="orb-picker" type="button" data-orb-picker="${index}" aria-label="Seleccionar orbe ${index + 1}" style="background-image:url('${slotImage}')">${selectedImage}</button><div class="orb-picker__menu" data-orb-menu="${index}" hidden></div></div>`;
  }).join('');
}
